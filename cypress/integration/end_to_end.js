describe('Pomo Timer', () => {
  beforeEach(() => {
    cy.visit('http://localhost:5500/source/index.html')
  })
  ...
  it('Check Pomo Timer', () => {
    ...
    const pomoLen = 120000
    const shortLen = 60000
    const fillerBar1MaxWidth = 215
    const fillerBar2MaxWidth = 44
    const fillerBarCurrentWidth = 382
    // cy.get('#progress-background')
    // .then(($el) => {
    //   fillerBarCurrentWidth = $el.width
    //   console.log(fillerBarCurrentWidth)
    // })
    // console.log(fillerBarCurrentWidth)
    ...
    cy.get('#filler-bar-1-svg')
      .invoke('attr', 'width')
      .then(($width) => {
        const calculatedWidth = ((1 - (pomoLen - 5000) / pomoLen) * fillerBar1MaxWidth) * screenMultiplier
    ...
    cy.get('#filler-bar-1-svg')
      .invoke('attr', 'width')
      .then(($width) => {
        const calculatedWidth = ((1 - (pomoLen - 48000) / pomoLen) * fillerBar1MaxWidth) * screenMultiplier
        const actualWidth = $width
        expect(Math.abs(calculatedWidth - actualWidth)).to.be.below(0.001)
      })
    ...
    cy.get('#filler-bar-1-svg')
      .invoke('attr', 'width')
      .then(($width) => {
        const calculatedWidth = ((1 - (pomoLen - 120000) / pomoLen) * fillerBar1MaxWidth) * screenMultiplier
        const actualWidth = $width
        expect(Math.abs(calculatedWidth - actualWidth)).to.be.below(0.001)
      })
    ...
    cy.get('#filler-bar-2-svg')
      .invoke('attr', 'width')
      .then(($width) => {
        const calculatedWidth = ((1 - (shortLen - 3000) / shortLen) * fillerBar2MaxWidth) * screenMultiplier
        const actualWidth = $width
        expect(Math.abs(calculatedWidth - actualWidth)).to.be.below(0.001)
      })
  ...
  it('Check state change of the timer', () => {
    ...
    const loop2 = Array.from({ length: 4 }, (v, k) => k + 1)
    cy.wrap(loop2).each(() => {
      cy.get('#time')
        .invoke('text')
        .then((text) => {
    ...
    // Long Break state
    cy.get('#progress-bar-background').then($el => {
      expect($el).to.have.attr('src', './assets/progressBarLongBreak.svg')
    })
    cy.get('#play').click()
    cy.wait(60000)
    cy.get('#time')
